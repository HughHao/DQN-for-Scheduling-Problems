...
f_list = [fitness(i) for i in population]
Xb, fb, fave = population[np.argmax(f_list)], max(f_list), np.mean([1/i for i in f_list])  # 最佳个体与平均适应度
best_fit, fit_ave = [], []
# 开始循环
start = datetime.datetime.now()
for i in range(iters):
    if i % 10 == 0:
        print('第{}次进化后的最优加工时间为{}'.format(i, 1/fb))  # 首个染色体的结束时间, solution_list含makespan函数和方法
    best_fit.append(1/fb)
    fit_ave.append(fave)
    pop_new = copy.deepcopy(population)
    # 将最佳的连个个体复制到新种群
    pop_new.sort(key=lambda x: 1/fitness(x))
    # f_a = [fitness(k) for k in pop_new]
    pop_children1 = pop_new[3::2]  # 偶数解，偶数组成的种群
    pop_children2 = pop_new[2::2]  # 奇数解
    # PMX两点交叉变异
    for i in range(len(pop_children1)):
        pop_children1[i], pop_children2[i] = two_points_cross(pop_children1[i], pop_children2[i])
    # 交叉后的子种群
    pop_new[2:] = pop_children1 + pop_children2
    # 变异
    for i in pop_new[2:]:
    ...
    f_list = [fitness(i) for i in population]  # 最小化问题的倒数，最大值
    Xb, fb, fave = population[np.argmax(f_list)], max(f_list), np.mean([1 / i for i in f_list])
    # 选择
    # 轮盘赌选择遗传个体
    ...
    population = copy.deepcopy(pop_new)
print('进化完成，最终最优加工时间为：', 1/fb)
